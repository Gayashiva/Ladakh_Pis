...
class timelapse(object):
  ...
  def print_stats(self):
    tmpl = 'SS: % 10d\tX: % 8.3f\tBR: % 4d\tISO: % 3d\tShots: % 5d'
    state = self.state
    x = self.config.SSToFloat(state.currentss)
    print tmpl % (state.currentss, round(x,2), state.lastbr, self.camera.ISO, state.shots_taken)
  ...
  def run_timelapse(self, config=None, state=None):
    ...
    while ((elapsed < config.maxtime or config.maxtime == -1) 
      ...
      #Take a picture.
      filename = ('/home/pi/AWS/Photos/%s.jpg' % (dtime))
...
def main(argv):
    ...
    try:
        os.listdir('/home/pi/AWS/Photos')
    except:
        os.mkdir('/home/pi/AWS/Photos')

    args=parser.parse_args()
    
    # # Change Night ISO
    # df_adc = pd.read_csv("/home/pi/AWS/adc6.csv", sep=",", header=0, parse_dates=["Datetime"])
    # if df_adc["UV"].iloc[-1] < 315:
      # args.iso = 800
      # print "ISO changed to 800"
